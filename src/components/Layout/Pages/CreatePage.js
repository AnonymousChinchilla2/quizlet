...
const CreatePage = ({ selectedItems, updateSelectedItems }) => {
  ...
  const renderLibraryContent = () => {
    return selectedItems.map((item, index) => (
      <div class='cardDropdown' key={`${item.type}-${index}`} className={styles.libraryItem}>
        <h2>{`${item.type.charAt(0).toUpperCase() + item.type.slice(1)}`}</h2>
        {renderLibraryImage(item.type)}
        <div class='dropdownOption' className={styles.itemOptions}>
  ...
  const renderLibraryImage = (option) => {
    switch (option) {
      case 'folder':
        return <img class='libraryImage' src={FolderImg} alt={`Library ${option}`} style={{ width: 100 }} />;
      case 'deck':
        return <img class='libraryImage' src={DeckImg} alt={`Library ${option}`} style={{ width: 100 }} />;
      case 'card':
        return <img class='libraryImage' src={CardImg} alt={`Library ${option}`} style={{ width: 100 }} />;
      default:
        return null;
    }
  };
